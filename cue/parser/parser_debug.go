...
func (p *parser) CuedoPrintNode(node ast.Node, c *commentState, remarks ...string) {
	if os.Getenv("CUEDO_AST_NODE_TYPE_AND_COMMENTS") != "" {
		...
		// prepare and print the node line including p.tok
		commentsFrom := ""
		if len(groups) > 0 {
			commentsFrom = sep + CuedoSetComments + " " + fmtAddress(c, true /*short*/)
		}
		p.printTrace(indentOther + prefix + sep + fmt.Sprintf("p.tok: %s", p.tok) + sep + address + commentsFrom + sep + callpoint(2, remarks...))
		prefix = strings.Repeat(" ", len(prefix))
...
// captureAndPrintStacktrace captures the stacktrace and prints it
func captureAndPrintStacktrace() {
	st := stacktrace.CapturedStacktrace()
	// trim off the uninteresting, repeating stacktrace lines
	st.Trim(8, 4)
	callpoints := st.StacklineCallpoints(true)
	length := st.Len()
...
func (p *parser) printCommentsStack(remarks ...string) {
	...
	addresses := p.commentsStackAddresses()
...
// commentStackAddresses returns the list of addresses of the parser's comments stack
func (p *parser) commentsStackAddresses() []*commentState {
...
func (p *parser) printCommentState(remarks ...string) {
	...
	if len(remarks) > 0 {
		...
		if len(remarks) > 0 {
			if remarks[0] == "pushed" {
				remarks[0] += fmt.Sprintf(" (%d)", len(p.commentsStackAddresses()))
			} else if remarks[0] == "popped" {
				remarks[0] += fmt.Sprintf(" (%d)", len(p.commentsStackAddresses())+1)
			}
		}
	}
