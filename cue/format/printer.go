...
func (p *printer) append(args ...interface{}) {
	...
	p.output = append(p.output, delta...)

	CuedoLogStackOneline()
	p.CuedoHexDump()
	p.CuedoHexDumpDelta(delta)
}
...
// CuedoHexDump logs the hexdump of the printer's output
func (p *printer) CuedoHexDumpDelta(delta []byte) {
	if os.Getenv("CUEDO_FORMATTER_HEXDUMP_DELTA") != "" {
		log.Printf("delta:\n%s\n", hex.Dump(delta))
	}
}
...
func CuedoLogStackOneline() {
	if os.Getenv("CUEDO_FORMATTER_STACKTRACE") != "" {
		...
		callpoints := st.StacklineCallpoints( /*short=*/ true)
